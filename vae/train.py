...
class Trainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_dict = self.network.loss_function(
        ...
        self.log('loss', loss, rank_zero_only=True, prog_bar=True)
        return loss
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='vae/train_solid.yaml')
    args = parser.parse_args()

    with open(args.config, 'r') as f:
        config = yaml.safe_load(f)

    network = VAE3D(**config['network_params'])
